...
export default {
  ...
  data() {
    return {
      ...
      selectedModel: null, // Will be set after fetching providers
      availableModels: [], // Dynamically fetched from backend
  ...
  methods: {
    ...
    async sendMessage() {
      ...
      try {
        ...
        // --- Add Model Params (Dynamic) ---
        if (this.selectedModel) {
            params.append("model_provider", this.selectedModel.provider);
            params.append("model", this.selectedModel.id);
        } else {
            // Fallback to gemini if no model selected
    ...
    async fetchProviders() {
      try {
        const response = await fetch(`${this.api_url}/api/providers`);
        if (response.ok) {
          const providers = await response.json();
          // Flatten all models from all providers into a single list
          this.availableModels = providers.flatMap(p => p.models);
          ...
          // Fallback to hardcoded gemini
          this.availableModels = [{ id: 'gemini-2.5-flash', name: 'Gemini 2.5 Flash', provider: 'gemini' }];
          this.selectedModel = this.availableModels[0];
        }
      } catch (e) {
